...
SELECT DISTINCT NODE, NODE_LEVEL FROM
    (SELECT N.NODE AS NODE, 
            ...
            C.NODE AS C_NODE,
            CASE 
            WHEN N.PARENT_NODE IS NULL THEN 'ROOT'
            WHEN C.NODE IS NULL AND N.PARENT_NODE IS NOT NULL THEN 'LEAF'
            WHEN N.PARENT_NODE IS NOT NULL AND C.NODE IS NOT NULL THEN 'INNER'
            END AS NODE_LEVEL
    FROM BST N
    LEFT OUTER JOIN BST C ON N.NODE = C.PARENT_NODE) AS RESULT
ORDER BY NODE;

-----------------------------------------------------------------------------------------
-- Contest Leaderboard
SELECT INNER_Q1.HACKER_ID, H.NAME, SUM(INNER_Q1.MAX_SCORE) AS TOTAL_SCORE
FROM
    (SELECT HACKER_ID, CHALLENGE_ID, MAX(SCORE) AS MAX_SCORE
    FROM SUBMISSIONS
    GROUP BY HACKER_ID, CHALLENGE_ID) AS INNER_Q1
LEFT OUTER JOIN HACKERS H ON H.HACKER_ID = INNER_Q1.HACKER_ID
GROUP BY HACKER_ID
